...
/* ---------- inti: satu state percakapan, dua kulit ---------- */

/**
 * Logika chat dipakai bersama oleh dua tampilan:
 *  - quick chat: widget mengambang di kanan bawah, modal tanpa daftar riwayat;
 *  - halaman /chat-ai: layar penuh dengan daftar riwayat di kiri.
 *
 * Keduanya membaca dan menulis SIMPANAN yang sama, jadi percakapan yang dimulai
 * dari quick chat muncul di halaman Chat AI, dan sebaliknya. Karena masing-masing
 * kulit punya salinan state sendiri, quick chat membaca ulang localStorage
 * (`segarkan`) tiap kali dibuka supaya daftarnya tidak basi.
 */
function usePercakapan(aktif: boolean) {
  ...
  async function tanya(isi: string) {
    ...
    try {
      ...
      // Disimpan dari mana pun percakapan ini dimulai — quick chat dan halaman
      // Chat AI berbagi satu daftar.
      const segar = muatThreads()
      const id = threadId ?? String(sekarang())
      setThreadId(id)
      const daftar = [
        { id, title: segar.find((x) => x.id === id)?.title ?? judulDari(t), updatedAt: sekarang(), messages: akhir },
        ...segar.filter((x) => x.id !== id),
      ]
      setThreads(daftar)
  ...
  /** Dipanggil saat kulit ini kembali terlihat: kulit satunya mungkin sudah menambah percakapan. */
  const segarkan = () => setThreads(muatThreads())

  return { status, threads, threadId, pesan, teks, setTeks, internet, setInternet, kirim, galat, baru, pilih, hapus, tanya, segarkan }
}

type Percakapan = ReturnType<typeof usePercakapan>

/* ---------- kulit bersama: panel percakapan ---------- */

function PanelChat({ c, onTutup }: { c: Percakapan; onTutup?: () => void }) {
  const bawah = useRef<HTMLDivElement>(null)
  useEffect(() => { bawah.current?.scrollIntoView({ behavior: 'smooth' }) }, [c.pesan, c.kirim])
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col">
      <div className="flex items-center gap-1.5 border-b border-[var(--border-subtle)] px-4 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold leading-tight text-white">
            {c.threads.find((t) => t.id === c.threadId)?.title || 'Percakapan baru'}
          </p>
          <p className="truncate text-[11px] text-zinc-500">
            hanya membaca data HydroGuard{c.status?.model ? ` — ${c.status.model}` : ''}
          </p>
        </div>
        <button onClick={c.baru} title="Percakapan baru" aria-label="percakapan baru" className="p-1 text-zinc-500 hover:text-zinc-300">
          <Plus className="h-4 w-4" />
        </button>
        {onTutup && (
          <button onClick={onTutup} title="Tutup" aria-label="tutup" className="p-1 text-zinc-500 hover:text-zinc-300">
            <X className="h-4 w-4" />
          </button>
        )}
      </div>

      {c.status && !c.status.siap && (
      ...
      <div className="flex-1 space-y-3.5 overflow-y-auto p-4">
        {c.pesan.length === 0 && !c.kirim && (
          <div className="space-y-2 py-6 text-center">
            <Bot className="mx-auto h-6 w-6 text-zinc-600" />
            <p className="mx-auto max-w-xl text-xs text-zinc-500">
        ...
        {c.galat && <p className="text-xs text-rose-400">{c.galat}</p>}
        <div ref={bawah} />
      </div>
      ...
      <div className="flex items-end gap-2 border-t border-[var(--border-subtle)] p-3">
        ...
        <button
          type="button"
          onClick={() => c.setInternet((v) => !v)}
          aria-pressed={c.internet}
          title={
            c.internet
        ...
        <textarea
          value={c.teks}
          onChange={(e) => c.setTeks(e.target.value)}
          onKeyDown={(e) => {
            // Enter mengirim; Shift+Enter baris baru.
            if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); c.tanya(c.teks) }
          }}
          rows={1}
          disabled={!c.status?.siap || c.kirim}
          placeholder={c.internet ? 'Tanya — rujukan luar ikut dicari…' : 'Tanya soal sensor, prediksi banjir, posko, atau sumber daya…'}
        ...
        />
        <button
          onClick={() => c.tanya(c.teks)}
          disabled={!c.status?.siap || c.kirim || !c.teks.trim()}
          aria-label="kirim"
...
/* ---------- quick chat: widget mengambang, modal, tanpa riwayat ---------- */

/**
 * Sengaja tanpa daftar riwayat: ini jalan pintas untuk satu pertanyaan cepat.
 * Percakapannya tetap tersimpan dan bisa dilanjutkan di halaman Chat AI.
 */
export default function ChatAI() {
  const [buka, setBuka] = useState(false)
  const c = usePercakapan(buka)
  ...
  if (!buka)
    return (
      <div className="bergoyang fixed bottom-5 right-5 z-40 w-56 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] p-3 shadow-lg shadow-black/40">
        ...
        <button
          onClick={() => { c.segarkan(); setBuka(true) }}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setBuka(false)}>
      <div
        role="dialog"
        aria-label="Quick chat HydroGuard AI"
        onClick={(e) => e.stopPropagation()}
        className="flex h-[min(44rem,calc(100vh-3rem))] w-[min(48rem,100%)] overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] shadow-2xl"
      >
        <PanelChat c={c} onTutup={() => setBuka(false)} />
      </div>
    </div>
  )
}

/* ---------- halaman Chat AI: layar penuh + riwayat ---------- */

/** Riwayat disimpan di peramban ini saja — belum ada backend. */
export function ChatAIPage() {
  const c = usePercakapan(true)
    ...
    <div className="flex min-h-0 flex-1 overflow-hidden">
      <aside className="hidden w-60 shrink-0 flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-inner)] md:flex">
        ...
        <div className="flex-1 overflow-y-auto p-1.5">
          {c.threads.length === 0 && <p className="px-2 py-1 text-xs text-zinc-600">belum ada percakapan</p>}
          {c.threads.map((t) => (
            <div
              key={t.id}
              className={cn(
                'group flex items-center gap-1 rounded-md px-2 py-1.5 hover:bg-zinc-900/40',
                t.id === c.threadId && 'bg-zinc-800/40',
              )}
            >
              <button
                onClick={() => c.pilih(t)}
